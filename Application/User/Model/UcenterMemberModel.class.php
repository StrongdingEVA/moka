...
//载入配置文件
require_cache(APP_PATH . 'User/Conf/config.php');
...
class UcenterMemberModel extends Model {
    ...
    protected $_skey_main = 'ucenter_member_{id}'; // uid(主键)获取账号缓存前缀
    protected $_skey_mobile = 'ucenter_member_mobile_{mobile}'; // 手机号获取账号缓存前缀
    protected $_skey_username = 'ucenter_member_username_{username}'; // 手机号获取账号缓存前缀
    ...
    public function resetPwd($uid, $pwd) {
        ...
        $user=  query_user(array('uid','password'),$uid);
        if($data['password']==$user['password']){
            return '新密码不能和旧密码一致';
        }
        clean_query_user_cache($uid, 'password');
        $rs = $this->where(array('id' => $uid))->save($data);
        return $rs ? TRUE : $this->getError();
    }
    ...
    public function saveData($data, $where) {
        ...
        //清缓存
        $where['id'] && S($this->getCacheKeyMain($where['id']), NULL);
