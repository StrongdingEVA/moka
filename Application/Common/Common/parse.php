...
function parse_emoticon($content)
{
    return preg_replace_callback("/(\\[.+?\\])/is", 'parse_emoticon_callback', $content);
}

function parse_emoticon_callback($data)
{
    if (preg_match("/#.+#/i", $data[0])) {
        return $data[0];
    }
    $check_img =  substr($data[0], 2, 3);
    $result = $data[0];
    if($check_img > 99 &&  $check_img< 205){
        $filepath = "/Theme/Green/Weiquan/Static/extend/tools/images/common/e".$check_img.".gif";
        $emoticon_src = "<img src='".$filepath."' />";
        $result = str_replace($data[0], $emoticon_src, $data[0]);
    }
    return $result;
}
