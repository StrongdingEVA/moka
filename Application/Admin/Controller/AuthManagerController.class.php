...
class AuthManagerController extends AdminController{
    ...
    public function user($group_id){
        ...
        $auth_group = D('AuthGroup')->where( array('status'=>array('egt','0'),'module'=>'admin','type'=>AuthGroupModel::TYPE_ADMIN) ) ->getfield('id,id,title,rules');
        $prefix   = C('DB_UCENTER')['db_prefix'];
        $l_table  = C('DB_UCENTER')['db_name'].'.'.$prefix.(AuthGroupModel::MEMBER);
        $r_table  = $prefix.(AuthGroupModel::AUTH_GROUP_ACCESS);
        $model    =  M('');
        $model->table( $l_table.' m' )->join ( $r_table.' a ON m.uid=a.uid' );
        $_REQUEST = array();
        $where = array('a.group_id'=>$group_id,'m.status'=>array('egt',0));
        $list = $this->lists($model, $where, 'm.uid asc', array(), 'm.uid,m.nickname,m.last_login_time,m.last_login_ip,m.status');
